...
def test_ragged_array() -> None:
    ...
    with pytest.raises(Exception):
        shapely.to_ragged_array(PO)  # type: ignore[arg-type] # pyright:ignore[reportArgumentType] # ty:ignore[invalid-argument-type] # pyrefly:ignore[bad-argument-type]
...
def test_wkb_module() -> None:
    ...
    with pytest.raises(Exception):
        shapely.wkb.dump(P, WkbWriter(), hex=True)  # type: ignore[call-overload] # pyright:ignore[reportCallIssue, reportArgumentType] # ty:ignore[no-matching-overload] # pyrefly:ignore[no-matching-overload]
    with pytest.raises(Exception):
        shapely.wkb.dump(P, WkbHexWriter(), hex=False)  # type: ignore[call-overload] # pyright:ignore[reportCallIssue, reportArgumentType] # ty:ignore[no-matching-overload] # pyrefly:ignore[no-matching-overload]
