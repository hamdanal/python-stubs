...
from shapely._typing import GeoArrayLike, GeoT, OptGeoArrayLike, SupportsGeoInterface
...
@overload  # fallback
def triangulate(geom: BaseGeometry, tolerance: float = 0.0, edges: bool = False) -> list[Polygon] | list[LineString]: ...
def voronoi_diagram(
    geom: BaseGeometry, envelope: BaseGeometry | None = None, tolerance: float = 0.0, edges: bool = False
) -> GeometryCollection: ...
def validate(geom: BaseGeometry) -> str: ...
def transform(func: Callable[[Unpack[_Ts]], tuple[Unpack[_Ts]]], geom: GeoT) -> GeoT: ...
def nearest_points(g1: BaseGeometry, g2: BaseGeometry) -> tuple[Point, Point]: ...
def snap(g1: GeoT, g2: BaseGeometry, tolerance: float) -> GeoT: ...
