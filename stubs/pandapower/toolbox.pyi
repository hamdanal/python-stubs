...
from numpy import bool_, floating, str_
...
@overload
def pq_from_cosphi(s: float, cosphi: float, qmode: _QMode, pmode: _PMode) -> tuple[float, float]: ...
@overload
def pq_from_cosphi(
...
) -> tuple[float, float] | tuple[NDArray[floating[Any]], NDArray[floating[Any]]]: ...
@overload
def cosphi_from_pq(p: float, q: float) -> tuple[float, float, _PMode, _QMode]: ...
@overload
def cosphi_from_pq(
    p: Iterable[float], q: float | Iterable[float]
) -> tuple[NDArray[floating[Any]], NDArray[floating[Any]], NDArray[str_], NDArray[str_]]: ...
@overload
def cosphi_from_pq(
...
def cosphi_from_pq(
    ...
    tuple[float, float, _PMode, _QMode] | tuple[NDArray[floating[Any]], NDArray[floating[Any]], NDArray[str_], NDArray[str_]]
): ...
def dataframes_equal(df1: DataFrame, df2: DataFrame, ignore_index_order: bool = True, **kwargs: Incomplete) -> bool: ...
def compare_arrays(x: NDArray[Any], y: NDArray[Any]) -> NDArray[bool_]: ...
...
def overloaded_lines(net: pandapowerNet, max_load: int = 100) -> Index: ...
def violated_buses(net: pandapowerNet, min_vm_pu: float, max_vm_pu: float) -> Index: ...
def nets_equal(
    net1: pandapowerNet,
    net2: pandapowerNet,
...
def merge_same_bus_generation_plants(
    net: pandapowerNet, add_info: bool = True, error: bool = True, gen_elms: list[str] = ...
) -> bool: ...
...
@overload
def get_element_index(net: pandapowerNet, element: str, name: str, exact_match: Literal[False]) -> Index: ...
@overload
def get_element_index(net: pandapowerNet, element: str, name: str, exact_match: bool = True) -> int | Index: ...
@overload
...
def next_bus(net: pandapowerNet, bus: int, element_id: int, et: str = ..., **kwargs: Incomplete) -> int: ...
...
def false_elm_links(net: pandapowerNet, elm: str, col: str, target_elm: str | Iterable[str]) -> Index: ...
def false_elm_links_loop(net: pandapowerNet, elms: Iterable[str] | None = None) -> dict[str, Index]: ...
def read_from_net(
    net: pandapowerNet,
    element: str,
...
def group_row(net: pandapowerNet, index: int, element_type: str) -> Series[Incomplete]: ...
def group_element_index(net: pandapowerNet, index: int, element_type: str) -> Index: ...
